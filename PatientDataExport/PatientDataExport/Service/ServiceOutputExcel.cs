...
namespace PatientDataExport
{
    public class ServiceOutputExcel
    {
        ...
        public string OutputExcel(string FilePath, Dic myDic, Dictionary<string, string> List_Disease)
        {
            ...
            //存储统计结果
            Excel.Workbook statisticsBook = myExcel.Workbooks.Add(true);
            Excel.Worksheet SexNumSheet = statisticsBook.Worksheets[1];
            ...
            //输出统计结果
            //输出处理的人数
            SexNumSheet.Cells[1, 3] = "统计的人数";
            SexNumSheet.Cells[1, 4] = myDic.NumAll;
            //男性人数人数
            SexNumSheet.Cells[1, 5] = "男";
            SexNumSheet.Cells[1, 6] = myDic.NumSex["男"];
            //女性人数
            SexNumSheet.Cells[1, 7] = "女";
            SexNumSheet.Cells[1, 8] = myDic.NumSex["女"];
            //不同年龄段的人数范围
            int tempCountRangeNum = 0;
            foreach (var eachRangeNum in myDic.NumAge)
            {
                SexNumSheet.Cells[2, tempCountRangeNum + 1] = eachRangeNum.Key.ToString();
                SexNumSheet.Cells[2, tempCountRangeNum + 2] = eachRangeNum.Value.ToString();
            ...
            {
                SexNumSheet.Cells[3, tempCountSexAgeNum + 1] = eachSexAgeNum.Key.ToString();
                SexNumSheet.Cells[3, tempCountSexAgeNum + 2] = eachSexAgeNum.Value.ToString();
                tempCountSexAgeNum = tempCountSexAgeNum + 2;
            }

            //总疾病的数量
            SexNumSheet.Cells[5, 1] = "总疾病的数量";
            SexNumSheet.Cells[5, 2] = myDic.DiseaseNum;

            //总ICD诊断数量
            SexNumSheet.Cells[5, 4] = "总ICD诊断的数量";
            int totalICDDiseaseNum = 0;
            SexNumSheet.Cells[5, 5] = myDic.ICDDiseaseNum;

            //总非ICD诊断数量
            SexNumSheet.Cells[5, 6] = "总非ICD诊断的数量";
            int totalNotICDDiseaseNum = 0;
            SexNumSheet.Cells[5, 7] = myDic.NotICDDiseaseNum;
            ...
            foreach (string preTitleSex in GroupSex)
            {
                ...
                {
                    SexNumSheet.Cells[6, titleRangeAll++] = preTitleSex + "，" + titleDisease;
                }
            }
            //筛选此范围内前50位的诊断，分男女
            //控制横向移动的变量
            ...
            foreach (string top20allSex in GroupSex)
            {
                ...
                try
                {
                    ...
                    if (top20male == null)
                    {
                        SexNumSheet.Cells[7, 2 + countX_Top20all] = "空白";
                        SexNumSheet.Cells[7, 3 + countX_Top20all] = "空白";
                    ...
                    else
                    {
                        ...
                        {
                            //做出统计
                            SexNumSheet.Cells[7 + countY_Top20all, 1 + countX_Top20all] = DiagnosisName(List_Disease, eachICDDiseaseNum.Key.ToString());
                            SexNumSheet.Cells[7 + countY_Top20all, 2 + countX_Top20all] = eachICDDiseaseNum.Key.ToString();
                            SexNumSheet.Cells[7 + countY_Top20all, 3 + countX_Top20all] = eachICDDiseaseNum.Value.ToString();
                            //写入所有的诊断列表
                ...
                {
                    SexNumSheet.Cells[7, 2] = "异常";
                    SexNumSheet.Cells[7, 3] = "异常";
            ...
            foreach (string eachSex in GroupSex)
            {
                foreach (string eachAge in GroupAge)
                {
                    ...
                    {
                        SexNumSheet.Cells[6, 7 + countX_titleSexAgeDisease++] = eachSex + "，" + eachAge + "，" + title_Disease;
                    }
                }
            }
            ...
            foreach (string eachSex in GroupSex)
            {
                ...
                foreach (string eachAge in GroupAge)
                {
                    ...
                    try
                    {
                        ...
                        {
                            //诊断名称
                            SexNumSheet.Cells[7 + countY2_SexAgeDisease, 7 + countX2_SexAgeDisease] = "空白";
                            //诊断编码
                            SexNumSheet.Cells[7 + countY2_SexAgeDisease, 8 + countX2_SexAgeDisease] = "空白";
                            //诊断出现的数量
                            SexNumSheet.Cells[7 + countY2_SexAgeDisease, 9 + countX2_SexAgeDisease] = "空白";
                        }
                        else
                        {
                            ...
                            {
                                //通过列表之中查找响应的疾病名称
                                SexNumSheet.Cells[7 + countY2_SexAgeDisease, 7 + countX2_SexAgeDisease] = DiagnosisName(List_Disease, eachDiseaseOfTop20DiseaseOfEachAge.Key.ToString());
                                //疾病的编码
                                SexNumSheet.Cells[7 + countY2_SexAgeDisease, 8 + countX2_SexAgeDisease] = eachDiseaseOfTop20DiseaseOfEachAge.Key.ToString();
                                //疾病的数量
                                SexNumSheet.Cells[7 + countY2_SexAgeDisease, 9 + countX2_SexAgeDisease] = eachDiseaseOfTop20DiseaseOfEachAge.Value.ToString();
                    ...
                    {
                        //诊断名称
                        SexNumSheet.Cells[7 + countY2_SexAgeDisease, 7 + countX2_SexAgeDisease] = "查询过程出现异常";
                        //诊断编码
                        SexNumSheet.Cells[7 + countY2_SexAgeDisease, 8 + countX2_SexAgeDisease] = "查询过程出现异常";
                        //诊断出现的数量
                        SexNumSheet.Cells[7 + countY2_SexAgeDisease, 9 + countX2_SexAgeDisease] = "查询过程出现异常";
                    }
